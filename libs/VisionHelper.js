...
function createVisionHelper(vision, color = 'white', division = 8,) {

    const fieldOfView = vision.fieldOfView;
    const range = vision.range;

    const geometry = new THREE.BufferGeometry();
    const material = new THREE.MeshBasicMaterial({ wireframe: true, color: color });
    ...
    for (let i = - foV05; i < foV05; i += step) {
        ...
        positions.push(Math.sin(i) * range, 0.01, Math.cos(i) * range);
        positions.push(Math.sin(i + step) * range, 0.01, Math.cos(i + step) * range);
