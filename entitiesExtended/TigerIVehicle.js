...
class TigerIVehicle extends YUKA.Vehicle {
    ...
    setBulletOnStart() {
        ...
        this._renderComponent.children[0].children[3].children[0].children[0].getWorldPosition(helperVector3) //gets position of gun's muzzle and copies it to helperVector
    ...
    start() {
        ...
        this.tankTurret = this._renderComponent.children[0].children[3];
        this.fireCube = this._renderComponent.children[0].children[3].children[0].children[0]
    }
    ...
    update(delta) {
        ...
        if (this.memoryRecords.length > 0 && !this.rotateToTargetPointBeforeBehavior) {
            ...
            if (record.visible === true) {
                ...
                //lets move a tankturret towards visible target when tank moves and when tanks stops
                let a = entity.position.clone()
                let b = this.position.clone()
                a.y = 0
                b.y = 0
                let v = a.sub(b)
                v.normalize()
                let p = new YUKA.Vector3()
                this.getDirection(p)
                p.y = 0


                //lets turn turret slowly to target if it became visible
                let targetQuat = this.tankTurret.quaternion.clone().setFromUnitVectors(p, v)
                this.tankTurret.quaternion.rotateTowards(targetQuat, 0.01)

                let tolerance = 0.009
                let ang = this.tankTurret.quaternion.angleTo(targetQuat);
                ang < tolerance ? (tankIsFacingTarget = true) : (tankIsFacingTarget = false)
                ...
                if ((this.velocity.length() < 0.001) && !this.hasFired && tankIsFacingTarget) {//sets bullet on fire start position
                    ...
                    if (ar[0]) {
                        ...
                        //////if has fired, lets remove trigger (this.stoppedForFire) and behavior will continue
                        this.stoppedForFire = false
                    }
        ...
        } else if (this.memoryRecords.length > 0 && this.rotateToTargetPointBeforeBehavior) {//let move a tankturret towards visible target when tank turns to arrivalPoint

            //lets move a tankturret towards visible target when tank rotates to targetPoint (AP)
            const record = this.memoryRecords[0];
            const entity = record.entity;
            ...
            //lets turn turret slowly to target if it becames visible
            let targetQuat = this.tankTurret.quaternion.clone().setFromUnitVectors(p, v)
            this.tankTurret.quaternion.rotateTowards(targetQuat, 0.01)
    ...
    checkHitSpot(entity) {//checks how badly target has been hit
        ...
        // if (entity.name === 'targetTigerI' && entity.side === this.enemySide) {//for type Pak38
        if (entity.side === this.enemySide) {//for type Pak38
